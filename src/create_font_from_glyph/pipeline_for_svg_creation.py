...
def download_image(image_url):
    ...
    try:
        ...
        with open(os.path.join(downloaded_images_dir, f"{file_name}{file_extension}"), 'wb') as f:
            f.write(image_data)
    except Exception as e:
        print(f"Error while downloading {image_name}: {e}")
    return os.path.join(downloaded_images_dir, f"{file_name}{file_extension}")
...
def get_image_output_path(cleaned_image, image_name, output_path, headlines):
    ...
    new_image_name = f"{glyph_name}_{int(right_edge - left_edge)}_{int(headline_starts -
                                                                       left_edge)}_{int(right_edge - headline_ends)}.png"
